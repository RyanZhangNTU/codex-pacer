...
The official packaged assets are:
...
Keep release messaging aligned with this scope so the project does not over-promise signing, notarization, or auto-update behavior beyond the supported stable workflow.
...
3. Build the unsigned Windows NSIS setup EXE.
...
- verify the Windows installer checksum and note that it is unsigned unless signing was separately configured
...
- Upload the unsigned Windows NSIS setup EXE when the Windows release script has been run.
...
- official release assets: signed and notarized macOS Apple Silicon DMG; unsigned Windows NSIS setup EXE
